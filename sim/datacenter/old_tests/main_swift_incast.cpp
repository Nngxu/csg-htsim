...
int main(int argc, char **argv) {
    ...
    SwiftRtxTimerScanner swiftRtxScanner(timeFromMs(1), eventlist);
    ...
    for (it = conns->connections.begin(); it!=conns->connections.end();it++){
        ...
        for (uint32_t dst_id = 0;dst_id<destinations->size();dst_id++){
            ...
            for (int connection=0;connection<1;connection++){
                ...
                // each src starts at random time, but subflows of a src start together
                double extra = extrastarttime * drand();
                for (int subflow = 0; subflow < 1; subflow++) {
#ifdef FAT_TREE
                    ...
                    // we can connect multiple times, once for each subflow
                    swiftSrc->connect(*routeout, *routein, *swiftSnk, timeFromSec(extra));
                }          
