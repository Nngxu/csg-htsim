...
int main(int argc, char **argv) {
    ...
    conns->save("incast.cm");

    ConnectionMatrix* cm = new ConnectionMatrix(0);
    if (cm->load("incast.cm"))
        cm->save("incast2.cm");
    ...
    NdpPullPacer* pacer = new NdpPullPacer(eventlist, linkspeed, 1 /*pull at line rate*/);   
    ...
    uint32_t long_dest_no = 1; //a dest on the same LS ans dst 0
    uint32_t long_src_no = 5;  //a source not on the same LS as dst 1
    if (no_of_nodes <= 128)
    ...
    NdpPullPacer* longpacer = new NdpPullPacer(eventlist, linkspeed, 1 /*pull at line rate*/);   
