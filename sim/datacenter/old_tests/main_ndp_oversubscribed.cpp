...
int main(int argc, char **argv) {
...
#ifdef OV_FAT_TREE
    OversubscribedFatTreeTopology* top = new OversubscribedFatTreeTopology(queuesize, &logfile, &eventlist,ff,COMPOSITE);
#endif
    ...
    for (int i=0; i<no_of_nodes; i++){
        is_dest[i] = 0;
        pacers.push_back(new NdpPullPacer(eventlist, HOST_NIC, 1));
