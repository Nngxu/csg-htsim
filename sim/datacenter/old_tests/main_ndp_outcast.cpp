...
int main(int argc, char **argv) {
    ...
    uint32_t no_of_conns = 0, cwnd = 15, no_of_nodes = DEFAULT_NODES, flowsize = 15;
    ...
    for (uint32_t c = 0; c<no_of_conns; c++){
        conns->addConnection(c+1,no_of_nodes-1-c);
    }
    
    if (outcast)
        conns->setOutcast(0,no_of_conns, no_of_nodes-no_of_conns);
    //conns->addConnection(1,no_of_nodes-1);
    ...
    for (uint32_t n = 0; n<no_of_nodes; n++)
        pacer[n] = new NdpPullPacer(eventlist,  linkspeed, 1 /*pull at line rate*/);
    
    map<uint32_t,vector<uint32_t>*>::iterator it;

    // used just to pruint32_t out stats data at the end
    ...
    for (it = conns->connections.begin(); it!=conns->connections.end();it++){
        ...
        for (uint32_t dst_id = 0;dst_id<destinations->size();dst_id++){
            ...
            for (uint32_t connection=0;connection<1;connection++){
                ...
                extrastarttime = 0;//src*200;//0 * drand();
