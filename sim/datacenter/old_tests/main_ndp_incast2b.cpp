...
int main(int argc, char **argv) {
    eventlist.setEndtime(timeFromUs(3000.0));
...
#ifdef FAT_TREE
    //    FatTreeTopology* top = new FatTreeTopology(no_of_nodes, linkspeed, queuesize, &logfile, 
    //                                       &eventlist,ff,COMPOSITE,0);
    FatTreeTopology* top = new FatTreeTopology(no_of_nodes, linkspeed, queuesize, NULL, 
                                               &eventlist,ff,COMPOSITE,0);
#endif
    ...
    NdpPullPacer* pacer = new NdpPullPacer(eventlist,  HOST_NIC, 0.99 /*pull at 99% line rate*/);   
        ...
        for (uint32_t dst_id = 0;dst_id<destinations->size();dst_id++){
            ...
            for (int connection=0;connection<1;connection++){
                ...
                if (connID == 1) {
                    ...
                    ndpSrc->connect(routeout, routein, *ndpSnk, timeFromSec(extra));
                }
