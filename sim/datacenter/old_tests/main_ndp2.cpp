...
int main(int argc, char **argv) {
	...
	if (!strcmp(argv[i],"-o")) {
		...
		i++;
	    } else if (!strcmp(argv[i+1], "ecmp_dnx")) {
		route_strategy = ECMP_FIB;
		path_entropy_size = 1;
		FatTreeSwitch::set_strategy(FatTreeSwitch::DNX);
	    }
	    i++;
	} else {
	    exit_error(argv[0]);
    ...
    FatTreeTopology* top = new FatTreeTopology(no_of_nodes, linkspeed, queuesize, qlf, 
					       &eventlist,NULL,qt,hop_latency,switch_latency);
#endif
	...
	if (route_strategy!=SINGLE_PATH && route_strategy!=ECMP_FIB){
	    ...
	    srctotor->push_back(top->switches_lp[top->HOST_POD_SWITCH(src)]);
	    ...
	    dsttotor->push_back(top->switches_lp[top->HOST_POD_SWITCH(dest)]);
