...
int main(int argc, char **argv) {
    ...
    uint32_t no_of_conns = 0, cwnd = 15, no_of_nodes = DEFAULT_NODES, flowsize = cwnd;
    ...
    simtime_picosec* start = new simtime_picosec[no_of_nodes]; 
    
    for (uint32_t i=0;i<no_of_nodes;i++){
        is_dest[i] = 0;
        start[i] = timeFromUs(100*drand());
    ...
    for (uint32_t n = 0; n < no_of_nodes; n++) {
        pacer[n] = new NdpPullPacer(eventlist,  linkspeed, 1);
    }
    ...
    for (it = conns->connections.begin(); it!=conns->connections.end();it++){
        ...
        for (uint32_t dst_id = 0;dst_id<destinations->size();dst_id++){
            ...
            for (uint32_t connection=0;connection<1;connection++){
                ...
                ndpSrc = new NdpSrc(NULL, NULL, eventlist,false);
                ...
                extrastarttime = slack * drand();//start[src];
          
                ndpSrc->connect(routeout, routein, *ndpSnk, extrastarttime);
