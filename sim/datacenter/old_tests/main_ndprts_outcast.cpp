...
int main(int argc, char **argv) {
    ...
    int no_of_conns = 0, cwnd = 15, no_of_nodes = DEFAULT_NODES,flowsize = 15;
    stringstream filename(ios_base::out);
    RouteStrategy route_strategy = NOT_SET;
    bool pace_rts = false, outcast = true;
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")){
            ...
            i++;
        } else if (!strcmp(argv[i],"-pace_rts")){
            pace_rts = true;
        }  else if (!strcmp(argv[i],"-no_outcast")){
            outcast = false;
