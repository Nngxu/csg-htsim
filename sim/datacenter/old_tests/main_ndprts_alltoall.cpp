...
int main(int argc, char **argv) {
    ...
    eventlist.setEndtime(timeFromUs((uint32_t)5000));
    ...
    while (i<argc) {
        if (!strcmp(argv[i],"-o")){
            ...
            slack = atoi(argv[i+1]);
            i++;
        } else if (!strcmp(argv[i],"-pace-rts")){
            pace_rts = true;
    ...
    NdpSinkLoggerSampling sinkLogger = NdpSinkLoggerSampling(timeFromMs(0.05), eventlist);
    ...
    cout << "Running alltoall with " << no_of_conns << " connections" << endl;

    //for (i = 0;i<no_of_conns;i++){
    //        conns->addConnection(i+1,no_of_nodes-1-i);
    //}
    //conns->setOutcast(0,no_of_conns, no_of_nodes-no_of_conns);
    ...
    conns->setManytoMany(no_of_conns);
    ...
    for (i = 0;i<no_of_nodes;i++)
        pacer[i] = new NdpPullPacer(eventlist,  linkspeed, 1 /*pull at line rate*/);


    NdpRTSPacer** rts_pacer = (NdpRTSPacer**)malloc(no_of_nodes*sizeof(NdpRTSPacer*));
    for (i = 0;i<no_of_nodes;i++)
        rts_pacer[i] = new NdpRTSPacer(eventlist,  linkspeed, 1 /*pull at line rate*/);
    ...
    for (it = conns->connections.begin(); it!=conns->connections.end();it++){
        ...
        for (uint32_t dst_id = 0;dst_id<destinations->size();dst_id++){
            ...
            for (int connection=0;connection<1;connection++){
                ...
                if (pace_rts)
                    ndpSrc = new NdpSrc(NULL, NULL, eventlist,true,rts_pacer[src]);                
                else
                    ndpSrc = new NdpSrc(NULL, NULL, eventlist,true,NULL);
                
                ndpSrc->setCwnd(cwnd*Packet::data_packet_size());
                ndpSrc->set_flowsize(flowsize*Packet::data_packet_size());
