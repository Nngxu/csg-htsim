...
int main(int argc, char **argv) {
    ...
    queuesize = queuesize*packet_size;
    srand(13);
    ...
    NdpSinkLoggerSampling sinkLogger = NdpSinkLoggerSampling(timeFromMs(10.0), eventlist);
    ...
    for (it = conns->connections.begin(); it!=conns->connections.end();it++){
        ...
        for (uint32_t dst_id = 0;dst_id<destinations->size();dst_id++){
            ...
            for (uint32_t connection=0;connection<1;connection++){
                ...
                ndpSrc->setCwnd(cwnd*packet_size);//Packet::data_packet_size());

                if (flowsize){
                    ndpSrc->set_flowsize(flowsize*packet_size);//Packet::data_packet_size());
                }
                
                ndp_srcs.push_back(ndpSrc);
                ndpSnk = new NdpSink(eventlist, linkspeed, 1 /*pull at line rate*/);
                ReorderBufferLoggerSampling* buf_logger = new ReorderBufferLoggerSampling(timeFromMs(10), eventlist);
